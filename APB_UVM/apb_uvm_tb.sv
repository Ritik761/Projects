...
// opertion mode selection
typedef enum bit [1:0]   {read_op = 0, write_op = 1, rst = 2} oper_mode;
...
// Sequence: WRITE followed immediately by a READ, repeated 15 times
...
// Sequence: drives 15 reset operations on the DUT
...
virtual apb_if vif;
    ...
    forever begin
      ...
      else if (vif.presetn && vif.psel && vif.penable && vif.pwrite)
         begin
          if (vif.pready) begin
            tr          = transaction::type_id::create("tr");
            tr.op       = write_op;
...
// Configuration object for the agent (controls active/passive mode)
class apb_config extends uvm_object; //configuration of env
...
module tb;
  ...
  apb_mem dut (.presetn(vif.presetn), .pclk(vif.pclk), .psel(vif.psel), .penable(vif.penable), .pwrite(vif.pwrite), .paddr(vif.paddr), .pwdata(vif.pwdata), .prdata(vif.prdata), .pready(vif.pready), .pslverr(vif.pslverr));
